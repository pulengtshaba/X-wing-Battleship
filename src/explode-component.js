...
/*
smoke effects, using additive and alpha blended particles together;
slowing down particles, bring smoke during an explosion.
*/
...
export const explode_component = (() => {
  ...
  class ExplodeEffect extends entity.Component {
    constructor(params) {
      ...
      //adding fire to the explosion site
    ...
    //after or during the explosion the ship/object exploded must be removed from the game
    Destroy() {
      this.particles_.Destroy();
      this.group_.parent.remove(this.group_);
    }
    //load the explosion sound, play it for the experience of an explosion
    InitEntity() {
    ...
    //this explosion should last only for a certain period of time
    Update(timeElapsed) {
  ...
  //small explosions that symbolize an attack on the ship
