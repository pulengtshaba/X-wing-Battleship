...
export const shields_controller = (() => {
  //Our vertex shader, used in making a shield that notifies main player about an attack
  const _SHIELD_VS = `
  ...
  //Our Fragment shader, used in making a shield that notifies main player about an attack
  const _SHIELD_FS = `
  varying vec3 vNormal;
  varying vec3 vNoiseCoords;
  varying vec3 vWorldPosition;
  ...
  class ShieldsController extends entity.Component {
    ...
    OnHit_() {
      ...
      if (this.Parent.Attributes.shields > 0) {
        //When the ship's shield is enabled, sound is used to alart the player
    ...
    //update the shield, so it's only visible when getting attacked
    Update(timeElapsed) {
