...
export const ammojs_component = (() => {
  ...
  class AmmoJSController extends entity.Component {
    ...
    EnemyDied() { 
      this._kills++;
      document.getElementById('KillText').innerText = this._kills;

      if(this._kills>= 10 && this._kills<15){
        document.getElementById('MissionTitle').innerText = 'Phase 2: Mild';
        document.getElementById('MissionText').innerText = 'Destroy All five(5)\n X-wing ships';
        document.getElementById('congratText').innerText = 'You passed\n Phase 1';
      }
      else if(this._kills>15){
        document.getElementById('MissionTitle').innerText = 'Phase 3: Hard';
        document.getElementById('MissionText').innerText = 'Destroy All nine(9)\n Turrets on \n StarDestoyer ship';
        document.getElementById('congratText').innerText = 'You passed\n Phase 2';
      }
      else if(this._kills==24){
        document.getElementById('congratText').innerText = 'You just finished\n The game';
      }
      else{
    ...
    /*
    doing a ray intersection query, where we cast a ray into this physics world, and if we get a hit,
    we'll get back the entity that got hit.
    */
    ...
    //remove an object that just exploded
    RemoveRigidBody(body) {
      this.EnemyDied();
      this.physicsWorld_.removeRigidBody(body.body_);
      body.Destroy();
    }
    //making a rigid body or an object
    ...
    //we can detect when we hit another ship,
    //and take damage if we get hit
