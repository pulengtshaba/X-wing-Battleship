...
export const turret_controller = (() => {
  ...
  const _MAX_TARGET_DISTANCE = 2500;
  const _MAX_ANGLE = 0.907;
  ...
  class TurretController extends entity.Component {
    ...
    //here we want Boss ship to be able to shot at the main player as the enemy
    Fire_() {
      ...
      p1.TotalLife = 2.0;
      p1.Width = 1.5;
        // when this boss ship fires a shot, sound must be heard
    ...
    //Raytest is used to detect hits with help from ammos physics engine
    UpdateShots_() {
    ...
    //The Boss ship needs to be able to detect if main player is within their radar, so it can be shot
    AcquireTarget_() {
    ...
    //Once the main player is within Boss radar, turrents need to follow it's track & take shots
    TrackTargets_(timeElapsed) {
      ...
      //When powerlevel is greater than power per shot, fire shots at closer target
      if (this.powerLevel_ >= _POWER_PER_SHOT) {
        if (Math.random() < 0.1) {
          this.Fire_();
        }
