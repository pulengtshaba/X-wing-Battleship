...
export const tie_fighter_controller = (() => {
  ...
  class TieFighterController extends entity.Component {
    ...
    //allows the enemy ship to fire any targets
    OnFire_() {
    ...
    UpdateShots_() {
      ...
      //detect hits for this ship with help from ammojs,ray intersection
      ...
      for (let s of this.shots_) {
        ...
        for (let h of hits) {
          ...
          s.Life = 0.0;
          //for each hit on the ship, tiny explosion should be shown
          const explosion = this.FindEntity('spawners').GetComponent('TinyExplosionSpawner')
          explosion.Spawn(h.position);    
        }
      }
    }
    //the ship should be able to heal after surviving an attack
    Update(timeElapsed) {
